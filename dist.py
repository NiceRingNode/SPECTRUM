...
def dist_skilled_forgery(features,feat_freq,template_num,nf,genuine_num,forgery_num):
...
def dist_random_forgery(features,feat_freq,template_num,nf,genuine_num):
    dist_positive,dist_negative,dist_template = [],[],[]
    dist_fpositive,dist_fnegative,dist_ftemplate = [],[],[]
    ...
    for i,feat_a in enumerate(features_anchor):
        ...
        for j in range(len(features_anchor)):
            if i != j:
                feat_n.append(features_positive[j][4])
                freq_feat_n.append(freq_features_positive[j][4])
