...
def scoreScatter(gen, forg):
    ax = plt.subplot()
    area = np.pi*4**2.2
    ax.scatter(gen[:,0],gen[:,1], s=area, color='#DC143C',edgecolors='gray',alpha=0.35,label='genuine')
    ax.scatter(forg[:,0],forg[:,1], s=area, color='deepskyblue',edgecolors='gray', alpha=0.35,label='forgery')
    ax.set_xlabel("S$_{min}$", fontsize=20)
    ax.set_ylabel("S$_{ave}$", fontsize=20)
    k = (np.sum(gen[:,1] / gen[:,0]) + np.sum(forg[:,1] / forg[:,0])) / (forg.shape[0] + gen.shape[0])
    x = np.linspace(0.03, 0.5, 500)  
    y = -x / k + 0.5
    plt.plot(x, y,linestyle='--', color='k',alpha=0.6, label='$S_{min}+S_{avg}=th$')
    plt.legend(loc =2)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    plt.title("Verification scores", fontsize=20)
    plt.show()
...
EER_G = []; EER_L = []
mEER_L = []
logger.info("For MCYT:")
DIST_P = np.load("log/seed%d/mcyt/dtw_dist_p%s.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/mcyt/dtw_dist_n%s.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/mcyt/dtw_dist_temp%s.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/mcyt/dtw_dist_fp%s.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/mcyt/dtw_dist_fn%s.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/mcyt/dtw_dist_ftemp%s.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
    ...
    for ii in range(n_users):   
        ...
        dmean_fn = np.mean(dist_fforgery[ii * n_test_f:(ii + 1) * n_test_f,0:template_num] + DIST_N[ii*n_test_f:(ii+1)*n_test_f,0:template_num],axis=1) / dmean / 2 / 2
...
EER_G = []; EER_L = []
logger.info("For BiosecurID:")
DIST_P = np.load("log/seed%d/bio/dtw_dist_p%s.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/bio/dtw_dist_n%s.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/bio/dtw_dist_temp%s.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/bio/dtw_dist_fp%s.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/bio/dtw_dist_fn%s.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/bio/dtw_dist_ftemp%s.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For Biosecure DS2:")
DIST_P = np.load("log/seed%d/bsds2/dtw_dist_p%s.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/bsds2/dtw_dist_n%s.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/bsds2/dtw_dist_temp%s.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/bsds2/dtw_dist_fp%s.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/bsds2/dtw_dist_fn%s.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/bsds2/dtw_dist_ftemp%s.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS2 w2:")
DIST_P = np.load("log/seed%d/ebio2/dtw_dist_p%s.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio2/dtw_dist_n%s.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio2/dtw_dist_temp%s.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio2/dtw_dist_fp%s.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio2/dtw_dist_fn%s.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio2/dtw_dist_ftemp%s.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS1 w1:")
DIST_P = np.load("log/seed%d/ebio1/dtw_dist_p%s_d0.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio1/dtw_dist_n%s_d0.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio1/dtw_dist_temp%s_d0.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio1/dtw_dist_fp%s_d0.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio1/dtw_dist_fn%s_d0.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio1/dtw_dist_ftemp%s_d0.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS1 w2:")
DIST_P = np.load("log/seed%d/ebio1/dtw_dist_p%s_d1.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio1/dtw_dist_n%s_d1.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio1/dtw_dist_temp%s_d1.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio1/dtw_dist_fp%s_d1.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio1/dtw_dist_fn%s_d1.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio1/dtw_dist_ftemp%s_d1.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS1 w3:")
DIST_P = np.load("log/seed%d/ebio1/dtw_dist_p%s_d2.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio1/dtw_dist_n%s_d2.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio1/dtw_dist_temp%s_d2.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio1/dtw_dist_fp%s_d2.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio1/dtw_dist_fn%s_d2.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio1/dtw_dist_ftemp%s_d2.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS1 w4:")
DIST_P = np.load("log/seed%d/ebio1/dtw_dist_p%s_d3.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio1/dtw_dist_n%s_d3.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio1/dtw_dist_temp%s_d3.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio1/dtw_dist_fp%s_d3.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio1/dtw_dist_fn%s_d3.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio1/dtw_dist_ftemp%s_d3.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
EER_G = []; EER_L = []
logger.info("For eBS DS1 w5:")
DIST_P = np.load("log/seed%d/ebio1/dtw_dist_p%s_d4.npy"%(seed, epoch))
DIST_N = np.load("log/seed%d/ebio1/dtw_dist_n%s_d4.npy"%(seed, epoch))
DIST_TEMP = np.load("log/seed%d/ebio1/dtw_dist_temp%s_d4.npy"%(seed, epoch))
dist_fgenuine = np.load("log/seed%d/ebio1/dtw_dist_fp%s_d4.npy"%(seed, epoch))
dist_fforgery = np.load("log/seed%d/ebio1/dtw_dist_fn%s_d4.npy"%(seed, epoch))
dist_ftemplate = np.load("log/seed%d/ebio1/dtw_dist_ftemp%s_d4.npy"%(seed, epoch))
if LOOP:
    for i in range(4):
        datum_p = []
        datum_n = []
...
user_all = np.array([100, 132, 140, 35, 35, 35, 35, 35, 35])
